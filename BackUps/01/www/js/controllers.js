...
.controller('AppCtrl', ['$scope', '$ionicModal', '$localStorage', '$http', 'baseURL', '$ionicLoading', '$ionicSlideBoxDelegate',
        function ($scope, $ionicModal, $localStorage, $http, baseURL, $ionicLoading, $ionicSlideBoxDelegate) {
    ...
    $ionicModal.fromTemplateUrl('templates/read.html', {
        scope: $scope
    }).then(function (modal) {
        $scope.modal = modal;
    });
...
//
    ...
    // -----------------
    //
    .controller('IndexController', ['$scope', '$http', '$localStorage', 'baseURL', '$ionicModal', '$ionicLoading', '$ionicSlideBoxDelegate',
                        function ($scope, $http, $localStorage, baseURL, $ionicModal, $ionicLoading, $ionicSlideBoxDelegate) {
        $scope.baseURL = baseURL;
        $scope.progressPages = {
            K1: 0,
            K2: 0,
    ...
    .controller('EntryController', ['$scope', '$location', '$http', '$localStorage', 'baseURL', '$ionicLoading',
                    function ($scope, $location, $http, $localStorage, baseURL, $ionicLoading) {
        $scope.baseURL = baseURL;
        var CURRENT_RECITAL_CODE = "CurrentRecitalCode";
        $scope.crc = "";
        ...
        $scope.recitalEntry = function () {
            ...
            if ($scope.recitals.recitalCode == $scope.crc) {
                ...
                $http.get(baseURL + 'recitals/' + $scope.recitals.recitalCode)
                    ...
                    .error(function (error) {
                        $ionicLoading.show({
                            template: '<ion-spinner icon="android"></ion-spinner><p style="color:azure">Recital Code Exists</p>',
                            duration: 3000
                        });
                    });
            }
